...
//	Author: 		noa1mbot	
//
//	This script is executable by the Speedrunner Tools addon, that will be called automatically on the round start within g_STLib.Funcs scope (so the identifier "g_STLib.Funcs" can be skipped).
//	Includes used for vs_st_speedrun in "../vscripts/st_scripts/include" folder.
//
...
Convars.SetValue("st_mr_ffa", 0);			// set 0 if you prefer to script a map solo (without other players on the server)
Convars.SetValue("st_allow_sdkhooks", 0);	// set 1 if you need OnEntityCreated and OnEntityDestroyed hooks in your script (disabled by default to lower a script workload on round_start) 
Convars.SetValue("sv_infinite_ammo", 0);
Convars.SetValue("nb_blind", 0);				// useful to check SI spawns
Convars.SetValue("god", 0);
// Convars.SetValue("host_timescale", 5);		// fast forward Intro cameras, if Inventory2 is used
// Convars.SetValue("director_must_create_all_scavenge_items", 1);		// spawn all potential items on the map
...
//
//	To skip the Intro cameras (legacy), use Inventory() instead.
//============================================================
function Inventory2()
{
	ZDump();							// dump near-saferoom zombies' pos/ang functions in "...left4dead2\ems\st_config\zdump.txt".
	DebugItems("molotov");				// dump all molotovs on the map
	DebugItems("pipe");					// dump all pipes on the map
	// DebugItems();						// dump all current in-map item spawns in "...left4dead2\ems\st_config\items_dump.txt".
	...
	if ((hPlayer = Ent("!nick")) != null)
	{
		...
		/*
			...
			4th arg:		bool			– do not teleport player at the start of the movement? (useless for EX mode)
		*/
	}
...
//============================================================
//	Called from Movement Reader: https://forums.alliedmods.net/showthread.php?t=309141
...
//============================================================
//	Called from ACB: https://forums.alliedmods.net/showthread.php?t=302498
...
//============================================================
//	Called on specified event. Place here your event callbacks with an unique name to get responce.
...
//============================================================
//	Called from Speedrunner Tools: https://forums.alliedmods.net/showthread.php?t=304789
//	When the entity is created or removed.
//	Use "st_allow_sdkhooks" to be able to use these callbacks. Note that OnEntityDestroyed doesn't receive a classname.
//
//		Parameters:
//	hEntity 		– entity that has been created or removed;
...
::OnEntityCreated <- function(hEntity, classname)
{
	...
	{
		printl("spawn: " + hEntity);
	}
}

::OnEntityDestroyed <- function(hEntity)
{
	if (hEntity.GetClassname() == "infected")
	{
		printl("removed: " + hEntity);
	}
}
...
Timer();				// countdown, that can be changed via !timer chat-command
