...
7. **按键注入方式**：移动/蹲起按键在 `str_replay_stuck_repro` 开启时（默认）仅合并到服务端 `buttons` 掩码以复现引擎卡住物理，关闭时通过 `ClientCommand` 注入；而动作键（+use/+attack/+attack2/+reload）对真人客户端除 `ClientCommand` 外还会合并进服务端掩码——`ClientCommand` 依赖客户端本机执行，对远程联机客户端不可靠，联机播放时其他客户端可能看不到按 E / 右键等动作，掩码由服务端直接判定并同步（门交互/推击/开火均为服务端逻辑）
